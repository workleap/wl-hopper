...
A select can include a search input to filter items in the dropdown. This is useful for long lists where users need to quickly find specific options. Use the `isSearchable` prop along with `inputValue` and `onInputChange` to enable filtering. The `useFilter` hook from `@hopper-ui/components` can be used to implement the filtering logic.

<Example src="select/docs/searchable" />
