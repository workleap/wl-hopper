...
3. If a prop is not in the whitelist → use propName directly (no `UNSAFE_` prefix)
4. Props with "Tokens Category" of "none" (like `overflow`, `flex`, `position`) are used directly
</AI>
...
<PropsReferenceTable
    rows={[
        ...
        ["gap", "gap", "spacing-scale", "breakpoints"],
