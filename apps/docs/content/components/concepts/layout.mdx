...
- `Inline` is `Flex` with `flex-direction: row`, `align-items: center`, and a default gap token for horizontal spacing.
...
It is fine to tweak one thing such as `gap`, `justifyContent`, or `wrap`. If you also need a different cross axis alignment and another structural change, prefer `Flex`.

Example:
```tsx
<Inline gap="inline-sm" justifyContent="space-between">
  <Title />
  <Actions />
</Inline>
```
...
It is fine to tweak one thing such as `gap` or `alignItems`. If you also need direction to flip at a breakpoint or multiple alignment rules, prefer `Flex`.
...
- Changing `alignItems` and `justifyContent` together,
