...
import { content, errorContent, getComponentDocumentation, getDocumentInfo, getGuideDocumentation } from "./utils.js";
...
export function tools(server: McpServer) {
    ...
    server.registerTool("get_component_props", {
        title: "Get component API as JSON",
        description:
    ...
    server.registerTool("get_guide", {
        ...
        return getGuideDocumentation(guide ?? "all");
    });
    ...
    server.registerTool("validate_code", {
        ...
        if (!valid) {
            ...
            return errorContent(error, "The code is invalid. Please fix the errors and try again.");
        }

        return content("I wasn't able to run the validations. This method is not implemented yet.");
    });
