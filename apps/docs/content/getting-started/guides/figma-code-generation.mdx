...
The Hopper MCP Server integrates with [Figma's MCP server](https://developers.figma.com/docs/figma-mcp-server/) to enable AI-powered code generation from Figma designs. The workflow analyzes your Figma frames and generates JSX code using Hopper components, design tokens, and best practices.

**Key Benefits**
- **Rapid prototyping**: Convert designs to code in minutes instead of hours
- **Design system compliance**: Automatically uses correct Hopper components and tokens
- **Consistency**: Reduces manual interpretation errors between design and code
- **High-fidelity output**: Generates code that closely matches Figma designs

<Callout variant="information">
    **Important:** For best results, ensure your Figma design follows Hopper's [Figma Conventions Guide](/ai/figma-conventions.md) for accurate component, icon, and token mapping during code generation.
</Callout>
