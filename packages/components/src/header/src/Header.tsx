...
function Header(props: HeaderProps, ref: ForwardedRef<HTMLElement>) {
    ...
    const classNames = clsx(GlobalHeaderCssSelector, stylingProps.className, className);
    ...
    return (
        <ClearProviders values={clearContexts}>
            <RACHeader ref={ref} className={classNames} style={mergedStyles} slot={slot || undefined} {...otherProps}>
                {children}
            </RACHeader>
        </ClearProviders>
    );
