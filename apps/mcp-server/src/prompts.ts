...
- [ ] Create a complete map of design tokens → component prop values for all used design tokens in the #get_code response by passing them to '#${toolsInfo.get_design_tokens_map.name}(${"all" satisfies TokenCategory})'tool.
- [ ] Call '#${toolsInfo.get_design_tokens_map.name} if needed to fill any gaps in the above mapping.
