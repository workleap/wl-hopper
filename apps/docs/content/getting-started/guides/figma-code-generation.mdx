...
<Callout>
    **To get best results** ensure your Figma design follows Hopper's [Figma Conventions
    Guide](/ai/figma-conventions.md) for accurate component, icon, and token mapping during code generation.
</Callout>
...
<Callout variant="information">
    **Why Claude Code?** Our testing shows that Claude Code produces the most consistent and accurate Hopper code from
    Figma designs. GitHub Copilot with Claude Sonnet 4.5 works well for simpler scenarios but tends to be slower. Codex
    has significantly slower performance and produces inconsistent results.
</Callout>
...
- **Get the Figma frame URL**
    ...
    <Callout variant="warning">
        **Frame size limitations:** Large or complex Figma frames may exceed your AI Assistant's context window size
        limits and cause the generation to fail. If this happens, break down your design into smaller frames and convert
        them separately.
    </Callout>
...
- **Prompt**
    ...
    ```
    <Callout variant="information">
        **Note for assistants without MCP prompt support:** If your AI assistant doesn't support MCP prompts (the
        `/generate_code_from_figma_design` command), you can copy the full prompt directly from the [Hopper MCP Server
        source code](https://github.com/workleap/wl-hopper/blob/main/apps/mcp-server/src/prompts.ts#L63-L149) and paste
        it into your assistant's chat, replacing the Figma URL placeholder with your actual design URL.
    </Callout>
...
- <PromptSnippet>
      Use "Chrome Dev Tools MCP server"'s "take_snapshot" tool to compare the generated code screenshot with the
      original Figma design, identify all visual differences, and fix them
  </PromptSnippet>
