...
| Do ✅ | Don't ❌ | Description |
|-------|----------|-------------|
| ```<Button><Text>Delete Item</Text><Icon /></Button>``` | ```<Button>text without text tag<Icon /></Button>``` | when you have icons it is required to wrap the button's text with `<Text/>` component.|
