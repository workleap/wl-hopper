...
>
When a Button contains anything other than a single text node (e.g., an icon, a spinner, or another element), the accompanying text must be wrapped in a `<Text>` component.
</DosAndDonts>
