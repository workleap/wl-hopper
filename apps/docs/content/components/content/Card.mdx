    ...
    data={[
        {
            Variant: "main",
            Purpose:
                "Main cards are used to set the first level of content of the page.  They use neutral-surface and border-weak tokens with no elevation. Most content is intended to be set in this card."
        },
        {
            Variant: "second-level",
            Purpose:
                "Second-level cards are used to group elements that are interactive within a main card with an overarching context, like a small form. They use a neutral-surface-weak token."
        }
    ]}
/>
