...
export interface AnonymousAvatarProps
    extends StyledSystemProps, AccessibleSlotProps, Omit<RenderProps<AnonymousAvatarRenderProps>, "children"> {
    /**
     * Whether or not the avatar is disabled.
     */
    isDisabled?: boolean;
...
function AnonymousAvatar(props: AnonymousAvatarProps, ref: ForwardedRef<HTMLDivElement>) {
    ...
    const { className, style, size: sizeValue, ...otherProps } = ownProps;

    const size = useResponsiveValue(sizeValue) ?? "md";

    const classNames = composeClassnameRenderProps(className, GlobalAnonymousAvatarCssSelector, stylingProps.className);
    ...
    return (
        <RichIconAvatarImage {...mergeProps(otherProps, renderProps)} ref={ref} size={size}>
            <AnonymousRichIcon />
        </RichIconAvatarImage>
    );
}
