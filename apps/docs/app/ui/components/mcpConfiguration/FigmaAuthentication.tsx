...
import { assistantIcons, assistantTitles, assistants } from "./assistants";
...
const FigmaAuthentication = () => {
    const claudeCodeContent = (
        <ul>
            <li>
                Type <code className="hd-code">/mcp</code> in Claude terminal to manage your MCP Servers and select
                figma
            </li>
    ...
    const vscodeContent = (
        <ul>
            <li>
                Open the Command Palette (<code className="hd-code">⌘ + Shift + P</code> on Mac,{" "}
                <code className="hd-code">Ctrl + Shift + P</code> on Windows/Linux)
            </li>
    ...
    const tabsContent = [claudeCodeContent, vscodeContent].map((content, index) => (
        <div key={assistants[index]}>{content}</div>
    ));
