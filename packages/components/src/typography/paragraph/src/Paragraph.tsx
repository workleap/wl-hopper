...
function Paragraph(props: ParagraphProps, ref: ForwardedRef<HTMLParagraphElement>) {
    ...
    const classNames = clsx(GlobalParagraphCssSelector, cssModule(styles, "hop-Paragraph", size), className);

    return (
        <Text ref={ref} elementType="p" className={classNames} size={sizeProp} {...otherProps}>
            {children}
        </Text>
    );
}
